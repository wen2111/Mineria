...
set.seed(1)
muestra <- sample(1:nrow(data), size = nrow(data)/3)
train<-data.frame(data_stand[-muestra,])
test<-data.frame(data_stand[muestra,])
...
matriz_confusion5 <- table(clase_predicha = prediccion, 
                          clase_real = data[muestra, "Exited"])
```
...
accuracy1 <- sum(diag(matriz_confusion1))/length(data[muestra,"Exited"])
sprintf("Accuracy1: %.2f%%", accuracy1*100)

accuracy5 <- sum(diag(matriz_confusion5))/length(data[muestra,"Exited"])
...
trainTransformed$Exited <- factor(ifelse(trainTransformed$Exited == "1","Yes","No"),
                                  levels = c("Yes","No"))
