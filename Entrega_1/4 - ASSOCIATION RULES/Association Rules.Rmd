...
```
Normalmente, a la vista del gráfico, se establecería un soporte mínimo de 0.05 o diferente dependiendo del caso de estudio. En este caso, el desbalanceo nos obliga a ser más permisivos.

Se es consciente de la problemática que supone el desbalanceo para la variable exited, por lo que será necesario aplicar un soporte mínimo **extremadamente bajo**. Se ha escogido 0.0125, y se fija una confianza también permisiva (0.7). De este modo se ponen unos filtros laxos para detectar reglas donde el consecuente sea que el cliente se va del banco (Exited=1) dada la circunstancia del desbalanceo.
...
```{r}
rules = apriori (data_tr, parameter = list (support=0.0125, confidence=0.7, maxlen = 5, minlen=2))
summary(rules)
```
...
No se han obtenido reglas que cumplan los parámetros de soporte mínimo 0.0125 y confianza 70%. Dado que los niveles se consideran como suficientemente flexibles para hacer frente al desbalanceo, se concluye que NO se puede encontrar mediante Association Rules reglas significativas que tengan como consecuente que el cliente abandone el banco
