...
df_AR_resumen <- data.frame(
  AR_1_10 = ifelse(
    rowSums(df_AR[, c("AR1","AR3","AR4","AR5","AR6","AR7","AR8","AR9","AR10")]) > 0,
    1, 0
  ),
  AR_11_20 = ifelse(
    rowSums(df_AR[, c("AR11","AR12","AR13","AR14","AR15","AR16","AR17","AR18","AR19","AR20")]) > 0,
    1, 0
  )
)
...
setwd("~/GitHub/Mineria/Entrega_2/Mario")
write.csv(submit, "submission_xgb_ar2.csv", row.names = FALSE)
